...
<head>
    ...
    <title>Manage Recipients - Organ Donation System</title>
...
<body>
    <div class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="/admin/recipients"><i class="fas fa-user-injured"></i> Recipients</a>
            <a class="nav-link" href="/admin/hospitals"><i class="fas fa-hospital"></i> Hospitals</a>
    ...
    <div class="main-content">
        <h2 class="mb-4">Manage Recipients</h2>
        <div class="table-container">
            <table class="table table-hover">
                <thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Blood Group</th><th>Required Organ</th><th>Urgency</th><th>Status</th></tr></thead>
                <tbody>
                    <c:forEach items="${recipients}" var="recipient">
                        <tr>
                            <td>${recipient.id}</td>
                            <td>${recipient.user.firstName} ${recipient.user.lastName}</td>
                            <td>${recipient.user.email}</td>
                            <td><span class="badge bg-danger">${recipient.bloodGroup}</span></td>
                            <td><span class="badge bg-primary">${recipient.requiredOrgan}</span></td>
                            <td>
                                <c:choose>
                                    <c:when test="${recipient.urgencyLevel == 'CRITICAL'}"><span class="badge bg-danger">${recipient.urgencyLevel}</span></c:when>
                                    <c:when test="${recipient.urgencyLevel == 'HIGH'}"><span class="badge bg-warning">${recipient.urgencyLevel}</span></c:when>
                                    <c:otherwise><span class="badge bg-success">${recipient.urgencyLevel}</span></c:otherwise>
                                </c:choose>
                            </td>
                            <td><span class="badge ${recipient.isActive ? 'bg-success' : 'bg-secondary'}">${recipient.isActive ? 'Active' : 'Inactive'}</span></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
